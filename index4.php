<?php
   $file = file_get_contents("task4.json");
...
<html>
   ...
   <body>
      <div class="wrapper ">
         <div class="mimezisi">
            <div class="row" >
               <div id="sad" style="position:absolute"><img src="4sad.gif"></div>
               <div id="happy" style="position:absolute" ><img src="4hepi.gif"></div>
            </div>
         ...
         <div>
            <?php
               echo "<form style='margin-left:5px; height= 100px;' name=\"edit\" action=\"action4.php\" method=\"GET\">";
               	...
               	foreach ($json_a as $item => $task) {
               		if ($task['status'] == "open") {	
               			...
               			echo " <a style='float: right; width:32px; height:30.5px; background-color:pink;margin-top:-6px; 
                              margin-right: -1px;border-width:1px;border-style: solid; border-color:pink; margin-right:-38px; ' href=\"action4.php?id=" .$item. "&action=delete\"></a>  ";
               			echo "</li>";}	}
               	if($havetasks > 0) {          
