...
if (empty($_GET['action'])) {
	echo"Error: What do you want me to do? \n<br /><a href=\"index3.php\">Go back and try again please.</a>";
...
#toon editformulier 
	...
	foreach ($json_a as $item => $task) {
		if ($item == $taskid) {
		$found = 1;
		echo "<form name=\"edit\" action=\"action3.php\" method=\"GET\">";
...
#update task
	...
	foreach ($json_a as $item => $task) {
		if ($item == $taskid) {
			...
			if(file_put_contents($file, $replaced, LOCK_EX)) {
				...
				echo"<a href=\"index3.php\">If that does not happen, please click here.</a>";
				?>
	...
	if ($found==0) {
		echo "Error. Task not found. <a href=\"index3.php\">Go back and try again please..</a>";
	}
	...
	if ($found==0) {
		echo "Error. Task not found. <a href=\"index3.php\">return.</a>";
	}
...
#delete task
	...
	if ($found==0) {
		echo "Error. Task not found. <a href=\"index3.php\">Please go back to the task list and try again.</a>";
	}
} else {
